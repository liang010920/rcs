    ...
    <section class="video-resource">
        <el-form :inline="true" :model="params" size="small">
            <el-form-item label="视频名称">
                <el-input v-model="params.name" placeholder="搜索视频"></el-input>
            </el-form-item>
        ...
        <el-row :gutter="15" style="margin-top: 10px" class="resourceManageHeight"
            ...
            <el-col :span="6" v-for="(resource, index) in resourceList" :key="'res'+index" style="margin-bottom: 15px">
                <el-card :body-style="{ padding: '15px'}" @click.native="selectRow(resource,index)"
                    :class="{selectResource:selectIndex.includes(index)}">
                    <el-image style="width: 100%; height: 200px;margin-bottom: 10px;"
                        :src="resource.image | imgPath('video')" fit="cover"></el-image>
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="35%" center :close-on-click-modal="false">
            <el-form ref="form" :model="dialogForm" label-width="80px" size="small">
                <el-form-item label="内容" class="is-required">
                    <upload-file-base v-if="dialogVisible" ref="videoUpload" accept="video/mp4,video/3gp,video/webm"
                        :limit="1" folderType="video" :fileSize="100" tip="视频大小不超过100M，.mpeg4、.3gp、.webm格式，最长90秒">
                    </upload-file-base>
                </el-form-item>
                <el-form-item label="封面">
                    <upload-file-base v-if="dialogVisible" ref="videoUploadImg"
                        accept="image/jpeg,image/png,image/gif,image/bmp" :limit="1" folderType="video" :fileSize="10"
                        tip="图片大小不超过10M，支持jpg/jepg、png、gif、bmp格式,图片大小建议：400*200"></upload-file-base>
                </el-form-item>
        ...
        <el-dialog title="预览" :visible.sync="previewDialogVisible" width="600px" center @close="closePreviewDialog"
            :append-to-body="true" :close-on-click-modal="false">
            <div>
                <el-card :body-style="{ padding: '15px'}">
                    <video width="518" height="292" controls autoplay controlslist="nodownload" ref="video"
                        disablePictureInPicture :src="preResource.file | imgPath('video')"
                        :poster="preResource.image | imgPath('video')"></video>
                    <!--<iframe height=498 width=510 src='https://player.youku.com/embed/XNDY3NzM0NDQ0NA==' frameborder=0 ></iframe>-->
                </el-card>
            </div>
        </el-dialog>
        <!-- preview dialog end-->
        ...
        <div class="page-wrap">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                :current-page="page.current" :page-sizes="[12, 16, 20, 32]" :page-size="page.size"
...
<script>
    ...
    export default {
        ...
        methods: {
            cancelRow(num) {
                if (num == 0) {
                    ...
                    let param = this.resParent.params
                    this.resParent.$emit('selectRow', num, param, this.checkIds)
                }
            },
            ...
            getGroupList() {
                this.resParent.getGroupList(this.params.groupType, val => {
                    this.groupList = val
                    this.selectAllGroupIndex = 0
                    for (var i = 0; i < this.groupList.length; i++) {
                        if (this.groupList[i].name == '未分组') {
                            this.catalogIdSelect = this.groupList[i].id
                        }
                        this.selectAllGroupIndex += this.groupList[i].itemCount
                    }
            ...
            loadData() {
                ...
                this.resParent.getPasswayListChild(data => {
                    param.chatbotId = data.records[0].id
            ...
            saveResource() {
                ...
                Object.assign(this.dialogForm, this.resParent.params)
            ...
            editResource(resource) {
                ...
                this.resParent.initData(resource.id, this.$refs, val => this.dialogForm = val)
            },
            previewResource(resource) {
                this.previewDialogVisible = true
                this.preResource = resource
                this.$nextTick(() => {
                    // this.$refs.video.src=resource.src
            ...
            closePreviewDialog() {
                // console.log(this.$refs.video.duration)//秒
        ...
        mounted() {
            ...
            this.resourceManageHeight = document.documentElement.clientHeight - 9 - 54 - 94 - 55 - 51 - 36 - 10 - 47 -
                42 -
                9 - 20 + 'px'
        },
        activated() {
