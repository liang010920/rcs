    ...
    <section class="media-list" @click="resetIndex--">
        ...
        <el-row>
            <el-col :span="scene == 1 ? 18 : 24" class="groupOverflow">
                <div class="group-item">
                    <el-button size="mini" :type="activeGroupType==-1?'primary':''" @click="switchGroup(-1)" round>全部
            ...
            <el-col :span="scene == 1 ? 6 : 0" style="text-align: right;" class="clearSpacing">
                <el-button v-if="scene==1" size="mini" round v-show="activeGroupType!=-1&&activeGroupType!=0"
                    @click="editGroup()">编辑分组</el-button>
                <el-button v-if="scene==1" size="mini" round class="mr10"
                    v-show="activeGroupType!=-1&&activeGroupType!=0" @click="deleteGroup()">删除分组</el-button>
                ...
                <el-popover v-if="scene==1" placement="bottom" width="200" v-model="popVisible">
                    ...
                    <el-button type="primary" size="mini" slot="reference" round
                        @click="popVisible = true;groupName='';catalogIdSelect = 0">新建分组</el-button>
                </el-popover>
            </el-col>
        </el-row>
        ...
        <el-row class="templateManageHeight" :style="{height:templateManageHeight}">
            <el-col :span="span" v-for="media in mediaList">
                <div class="media-col">
                    <media-item :preData="media" :resetIndex="resetIndex" :scene="scene" v-bind="$attrs"
                        @delMedia="delMedia" @editMedia="editMedia" v-on="$listeners"></media-item>
                </div>
            </el-col>
        </el-row>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.templateManageHeight = document.documentElement.clientHeight - 50 - 50 - 45 - 55 - 55 - 36 - 15 - 32 +
                'px'
            if (this.scene == 2) {
